...
%
% dddp was mislabeled as dpdd before 6/30/00.  The code worked properly (and continues
% to work), but this variable is now correctly labeled, and the units are correct.

%phase_name=label1(17:28,:);            % phase names
p    = header1(10,:)*180/pi;           % ray parameter (s/rad)
dddp = header1(11,:);                  % ddelta/dray_parameter (rad^2/s)
...
R = rad_src.*rad_rec./vsrc .* sqrt(abs(cos(take_off_rec).*cos(take_off_src).*sin(delta).*dddp./p));
