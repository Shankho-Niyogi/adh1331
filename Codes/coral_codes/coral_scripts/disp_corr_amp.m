% This does not work properly yet.  There is a bug somewhere!!!
...
VEL=iasp91(RADIUS);
RAYPARAM=HEADER(10,:);
UP_DOWN=(HEADER(12,:)>0);                             % =1 for up, 0 for down
TAKE_OFF=(asin(RAYPARAM*VEL/RADIUS*180/pi)*180/pi)';
AZIM    =Azim(WIN_INDEX)'+UP_DOWN'.*180;               % if ray is up plot at azim+180
RAD_PAT_CORR=radpattern(M(12:23), TAKE_OFF, AZIM, 1)
MAX_P_TO_P_CORR=MAX_P_TO_P./abs(RAD_PAT_CORR);
...
semilogy(EPI_DIST,MAX_P_TO_P_CORR,'o',EPI_DIST,MAX_P_TO_P,'*');            % plot amplitudes
