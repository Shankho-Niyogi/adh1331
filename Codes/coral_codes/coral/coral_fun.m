...
%    coral_fun        function version of coral
% USAGE:  [out_opts, out_matrix, out_phase, out_station, opts] = coral_fun(opts);
%    Coral version 2.2     2/17/2000  - MATLAB 5.2
%   coral     MATLAB function to analyze seismogram record sections 
...
%
% the function version has the input structure opts
% opts.quakefile is the name of the earthquake catalog read by 'quak'
% opts.datafile is the name of the AH data file read using 'read'
% opts.scriptfile is the name of the m-file macro run using 'm '
% opts.eval_at_end is a character string that gets executed using 'eval' at end
%
% out_opts is a structure containing the differential time picks
% out_matrix, out_phase, out_station are arrays that are described in coral_fun_end
% the output of coral should be modified to be more general, perhaps dirven by 
% other arguments in opts

global h_plot h_tt

%there are two versions of coral, a script version and a function version
% the only differences are that the top line should be commented out for the script version
% if the name of this program contains the letters 'fun' then it is assumed to be the function version of coral
if findstr('fun',mfilename); 
  coralfun=1; 
  out_opts=[];
  out_matrix=[];
  out_phase=[];
...
if ~exist('CORALHOME'),       CORALHOME=[];                      end
if length(CORALHOME)==0;
  CORALHOME   = which('coral');                 % directory containing the coral code
  INDCORALHOME= findstr('/coral/',CORALHOME);
  CORALHOME   = CORALHOME(1:INDCORALHOME-1);    % strip out the coral directory to get to the base path
end
...
disp('              Coral      Version 2.2      2/17/2000')
disp(' Type ''help'' for a list of commands and help command name for help on a specific command')
disp(' e.g. ''help read'' or ''help orga'' for general help on orginazation of coral')
...
if coralfun; % get earthquke catalog, AH file name, and a macro  to run
  if nargin==0; opts=[]; end
  [opts, new_opt, n_new_opt, i_new_opt] = coral_fun_start(opts); 
end
...
while STOP==0, 
 ...
 elseif abs(opt(1:4))==abs('help')
   ...
   if n_smat>3,
     ...
     coral_help([ CORALHOME '/coral/coral.man'],command_name);
   end
...
%        qtimes(1,:) = qtimes(1,:)+1900*(qtimes(1,:)<100);
 ...
 elseif abs(opt(1:4))==abs('read')
   ...
   if n_smat~=2,
     ...
     file_read='t';
     if exist(filename)==0
       if strcmp(filename,'fmi');
         disp('converting data from FMI')
         if exist('seiswav') & exist('seisatt') & ...
            exist('channelinfo') & exist('event')
           [Station,Loc,Calib,Comment,Record,Extras,Data] = fmi2coral...
           (seiswav,seisatt,channelinfo,event);
           clear Mag
         else
           file_read='f';
           disp(['Can''t convert data from FMI, try again']);
         end
       elseif exist([filename '.mat'])==0
         file_read='f';
         disp(['the file: ' filename ' does not exist, try again']);
       else
       ...
       if findstr(lower(filename),'.sac') == length(filename)-3; % file name ends in .sac so read it as a sac file
         [ok,Station,Loc,Calib,Comment,Record,Extras,Data,ray_stuff]=sac2coral(filename);
       else
         [Station,Loc,Calib,Comment,Record,Extras,Data] = ah2ml(filename);
       end
       % Data sometimes are asigned as complex, taking up twice the needed space
       % if Data is complex, and none of the elements have a non-zero imaginary 
       % part then force Data to be real 
       if ~isreal(Data);  if ~any(imag(Data(:))); Data=real(Data); end; end;
       if length(Station)==0;
         disp(['the file: ' filename ' does not contain data in the AH format, try again']);
         file_read='f';
       else
         CC=setstr(Comment); 
         ...
         % left justify Station name, Channel name and Network code
         indx=2:6;  Station(indx,:) = abs(left_justify( setstr( Station(indx,:)' ) )' );
         indx=8:12; Station(indx,:) = abs(left_justify( setstr( Station(indx,:)' ) )' );
     ...
     if file_read=='t'
       ...
       if (Loc(7,1)==0 | Loc(7,1)==1900) & Loc(8,1)==0,
         disp(' No event origin time is given in the AH header')
         % origin time of earthquake has date and time equal to zero
         % try to fix this

         stimes=Record(1:2,:);                 % start times for each seismogram
         [tmp,ind]=min(timediff(stimes));      % index to earliest time
         stime =stimes(:,ind);                 % earliest start time (date,time format)
         etimes=timeadd(stimes,Record(3,:).*Record(4,:)); % end times for each seismogram
         [tmp,ind]=max(timediff(etimes));      % index to latest time
         etime =etimes(:,ind);                 % latest end time (date,time format)
         n_records=size(Loc,2);                % number of seismograms

         if exist('qtimes')==1,                % an earthquake catalog was read in

           % choose event from catalog based on start and end times of seismograms
           ...
           disp('Origin time is set to time of first record and location to 0,0,0')
           % no earthquake catalog was entered, set origin time to start time of first 
           % seismogram and event location to 0,0
           ev_time=stime;     
           ev_loc=[0,0,0,0]';  % lat, lon, depth, magnitude
         ...
         disp('See command orig to reset the location properly:')
         ev_time6 = time_reformat(ev_time);
         disp(sprintf('orig %8.3f%9.3f%6.1f %4d %2d %2d  %2d %2d %7.2f %3.1f', ev_loc(1:3), ev_time6, ev_loc(4)))
         file_read='t'; 
       end
       if file_read=='t';
...
% JCC1
   ...
   if n_smat~=2,
     ...
     out_filename=cut_string(smat(2,:));
     eval (['save ',out_filename,' Azim Bakazim Calib Comment Data Delta Extras Header Label Loc Obs Record STOP Sintr Station Syn Tstart data1 header1 label1 obs1 titl label_key index w_index'])
   end
% JCC2    
 ...
 elseif abs(opt(1:2))==abs('m ')
   ...
   [smat,n_smat]=cut_string(opt);
   if n_smat<2,
     disp('m must have at least one argument, (name of command file) try again'); 
   else
 ...
 elseif abs(opt(1:2))==abs('tt')
   ...
   if n_smat==1,
     ...
     elseif phases_tt(1,1:5)=='label' & n_smat==3
       ...
       tmp=phases_tt';
       disp(['phases added to the synthetics: ' tmp(:)'])
       depth=Loc(6,1);
 ...
 elseif abs(opt(1:4))==abs('sort')
   ...
   if sort_from>=0
     ...
     elseif findstr(temp,'azim');
       temp1=Azim;   % [temp1,sort_key]=sort(Azim);
     elseif findstr(temp,'back');
       temp1=Bakazim; % [temp1,sort_key]=sort(Bakazim);
...
% JCC1
 elseif abs(opt(1:4))==abs('beam')
   % plot a map of stations and source
   % default = color, black and white if input is 2
   [beam_time,beam] = make_beam_ares(data1,header1);
% JCC2
   
 elseif strcmp(opt(1:2),'rm') | strcmp(opt(1:4),'keep')  
   % remove seismograms from Data 0 or data1
   rm_from = rm_sta(opt);  % decide whether to act of data0 or data1

   if rm_from==0,
     [rm_from,keep_key]=rm_sta(opt,Calib,Station,Header,Delta,Azim,Bakazim);
     m_data=size(Data,2);
   ...
   elseif rm_from==1,
     if exist('header1'),
       if length(header1)>0,
         [rm_from,keep_key]=rm_sta(opt,Calib,Station,header1,Delta,Azim,Bakazim);
         m_data=size(data1,2);
         if length(keep_key)<m_data & length(keep_key)>0
       ...
       disp('There is no data in data1')
     end
   end
 ...
 elseif abs(opt(1:4))==abs('corr')
   ...
   elseif n_smat==2,
     ...
     cmt_file_name=[ CORALHOME '/quakes/cmt.mat'];
   end
   ...
   if n_smat<2,
    ...
    if cut_from~=0 & cut_from~=1,
     ...
     if (max(abs(Loc(6,:)-Loc(6,1)))) > 0,
       ...
       for iph=1:n_phase
         phs_nam=phases(iph,:);    %force phs_nam to be 6 characters long
...
elseif abs(opt(1:4))==abs('tren')
 ...
 % plot record section
   ...
   if n_smat~=2
     ...
     if plot_option(1)=='0'
       my_rsx(plt_offset,Data,yval,Header,Syn,plot_scale,titl,ylab,Label,0,[],syn_label);
     elseif plot_option(1)=='1' 
       [window,button,nskip,data1,header1]= ...
       my_rsx(plt_offset,data1,yval,header1,Syn,plot_scale,titl,ylab,label1,1,[],syn_label);
       window=window';
       if prnt>=2; window, end
       wind_save=window;
       if button==13 & length(window)>0;
         size(window,2),size(header1,2),max(abs(window(1,:)))
         if size(window,2)==size(header1,2) & max(abs(window(1,:)))>0
     ...
     elseif plot_option(1)=='2' 
       my_rsx(plt_offset,data2,yval,header2,Syn,plot_scale,titl,ylab,label2,0,[],syn_label);
     elseif plot_option(1)=='3' 
       my_rsx(plt_offset,data3,yval,header3,Syn,plot_scale,titl,ylab,label3,0,[],syn_label);
     else
...
elseif abs(opt(1:4))==abs('resa')
 % resample data to a common value
   [smat,n_smat]=cut_string(opt);
   if n_smat==1, % default is to resample to largest sample interval
     [data1,header1,resam_opt] = resamp_part(data1,header1);
   else
     [resam0,resam_count,ERRMSG]=sscanf(opt, ... 
     '%*s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f');
     resam_opt.sintr_target=resam0(1);
	 [data1,header1,resam_opt] = resamp_part (data1,header1,resam_opt);
   end
...
elseif abs(opt(1:4))==abs('diff')
 ...
 elseif abs(opt(1:4))==abs('titl')
   ...
   if n_smat<2
     ...
     if deblank(titl)=='a'; % title will be date/time depth, magnitude, component of first record
       titl_flag='a';
       if exist('Loc')==1
         titl=set_title(Loc(:,1),Station(:,1),Mag(:,1))
       end
 ...
 elseif abs(opt(1:4))==abs('yaxi')
   ...
   if n_smat==2,
     temp=smat(2,:);
     if temp(1)=='e'|temp(1)=='d'|temp(1)=='s'|temp(1)=='E'|temp(1)=='D'|temp(1)„ÿ hOhÍ:jšŒš„²ãìÒ¸=Ÿ½RW3wİœßÄß7º‡†ï§Òí£ŸQ€lÌØIXğsê+áŸŠ?|InaÔ"¶$í ;_áQßëZ¨é©/aú|IkuÚ]Ï¼½7İóÖ¾¡ğ·…¯uÅ¶mBax÷™¥ÉUã¡ÇOÂ“V!îs¾:ğÕ–›¨ñrá$Î"uÃ†mAÜzWeğ{Ã'ºQ¨Ü=³¸ßæçĞì}(rãáÂC4_‰îxeWÌF:ÙOzã#»‹O»†IˆKrÌ¹q/ÓŞ‹sİ<-¦x,hPÜÜF³]ó,;
