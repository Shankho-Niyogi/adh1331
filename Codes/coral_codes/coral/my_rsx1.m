function [window,button,nskip,d,header,my_rsx_opts]=my_rsx(t,d,delta,header,syn,scale, ...
                            titl,ylab,tlab,pick,windowin,Syn_label);
%   my_rsx        record section plotter
% usage: [window,button,nskip,d,header,my_rsx_opts]=my_rsx(t,d,delta,header,syn,scale, ...
%                           titl,ylab,tlab,pick,windowin,Syn_label);
%
...
if length(tlab(:,1)) == 30,
  ...
  text(xval,yval,temp1','FontSize',font_size);
%  h_text=text(xval,yval,temp1');set(h_text,'FontSize',font_size);
else 
  text(xval,yval+.1,tlab','FontSize',font_size);
    ...
    if button==1 | button==2 | button==3;      % entered first new window limit
      ...
      while (button == 1 | button == 2 | button == 3),
        ...
        if wflag == 0,
          wflag = 1;
          echo_ml(' ')
          echo_ml(' Select one of limits for each trace of interest using','-n')
          echo_ml(' one of the mouse buttons')
          echo_ml('     LB -> set left limit, RB -> right limit, CB -> center,')
          echo_ml('     ''c'' to copy window to all traces')
        else
          echo_ml(' ')
          echo_ml('    Select next trace, <CR> to analyze traces,','-n')
          echo_ml(' ''r'' to replot,')
        end
        [xx,yy,button]=ginput(1), if length(button)==0; button=13; end
      end;
      ...
      if wflag == 0,                          % forgot to enter second limit...
        echo_ml(' ')
        echo_ml('Middle or Right mouse button must be used to enter')
        echo_ml('   second limit after first limit is entered, start again')
      elseif button == 13,
        done='t';
     elseif button == 'P',
        orient tall
...
%      size(t_in),size(d),size(delta),size(header),size(syn),
%      size(scale),size(titl),size(ylab),size(tlab),0,size(window),size(Syn_label)
      figure(2)
      hold off
      [ndata,ntrace]=size(d);

      tim_resid      =zeros(ntrace,1);
      qual_tim_resid =zeros(ntrace,1);
A=[ 1.8972    1.0000
    0.0198    3.0000
    2.1443    1.0000
    2.9348    1.0000
   ...
   -0.4743    3.0000
    ...
    1.4526    3.0000
    1.4526    3.0000];
   %   tim_resid=A(:,1);qual_tim_resid=A(:,2);
      for traceno = 1:ntrace  
        my_rsx1(t_in,d(:,traceno),delta(traceno),header(:,traceno),syn, ... 
               scale,titl,ylab,tlab(:,traceno),0,window,Syn_label);
        [XP,YP,button]=ginput(1)
        tim_resid(traceno)=XP;
        qual_tim_resid(traceno)=button;
      end
      ...
      I=find(qual_tim_resid==1 | qual_tim_resid==2);
      window_mv=window_mv(I,:);
      window_mv(:,1) = window_mv(:,1) + tim_resid(I); 
      window=window_mv;
      my_rsx_opts.pick = 'absolute';
