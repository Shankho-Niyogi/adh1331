...
if length(windowin) > 0,
  if length(windowin(1,:)) < 3,
    disp(' ')
    disp('Improper definition of input windows, ignoring input windows...')
    windowin = [];
  end;
end;
...
%   one window is selected.
    ...
    if button==1 | button==2 | button==3;      % entered first new window limit
      ...
      traceno = tempindx(1);
      disp(' ')
      disp('Select second limit for this trace with middle')
      disp('     or right mouse button...')
      [xx,yy,button]=ginput(1); if length(button)==0; button=13; end
...
% exit and skip n traces...
    ...
    elseif setstr(button) == 's',
      disp(' ')
      disp('In plot window, enter number of records to skip: ');
      disp('     (press <return> to indicate end of integer):      ');
      nskip = ginput_num;
      if nskip >= 0,
        nskip = nskip - 1;
      else,
...
% exit and go to nth trace...
    ...
    elseif setstr(button) == 'g',
      disp(' ')
      disp('In plot window, enter trace number you wish to analyze');
      disp('     (press <return> to indicate end of integer):      ');
      nskip = ginput_num;
      done='t';
...
% flip trace indicated by mouse input...
    ...
    elseif setstr(button) == 'f',
      disp(' ')
      disp('Select traces to flip with mouse (<CR> to exit flip mode)')
      temp=1;
      while temp~=13,
%        [x, y, temp] = ginput(1); if length(temp)==0; temp=13; end
...
% JCC End
        ...
        elseif temp~=13
          disp('In flip mode you must enter a mouse button or <CR>');
        end
      end
%      hold off;
...
% kill (remove) trace indicated by mouse input...
    ...
    elseif setstr(button) == 'k',
      disp(' ')
      disp('Select traces to remove with mouse (each mouse ckick removes a trace)')
      disp('<CR> removes selected traces from D1, ''q'' undoes all data removals')
      if length(window)==0, 
        [ndata,ntrace]=size(d);
      ...
      while temp~=13 & temp~=113,      % quit if button = <CR> or 'q'
        ...
        elseif temp~=13 & temp~=113
          disp('in data remove mode you must enter a mouse button, <CR>, or ''q''');
        end
      end
      if temp==13, 
      ...
      if temp==113,
        disp('Quitting trace remove mode, no traces removed, enter new command') 
      end
...
% deglitch trace indicated by mouse input...
    ...
    elseif setstr(button) == 'd'
      temp = [];
      disp(' ')
      disp('Select trace to deglitch with right or left mouse button...')
      [x, y, temp] = ginput(1); if length(temp)==0; temp=13; end
      if temp == 3,
        [tempx, tempi] = sort(abs(delta - y));
        traceno = tempi(1);
      ...
      elseif temp == 1;
        ...
        traceno = tempi(1);
        disp(' ')
        disp('  In plot window, enter number of glitches to remove');
        disp('     (press <return> to indicate end of integer):      ');
        nglitch = ginput_num;
...
% help, gives list of possible commands...
    ...
    elseif setstr(button) == 'h',  % help, gives list of possible commands
      disp(' ')
      disp('The following commands exit RSX (return to calling routine):')
      disp('  n    go to next record')
      disp('  b    go back to last record')
      disp('  r    replot current record')
      disp('  q    exit and quit')
      disp(' <CR>  exit with current window')
      disp(' ')
      disp('The following request a number in command window then exit RSX:')
      disp('  s    skip n records (must enter n then <CR>; can be negative)')
      disp('  g    go to nth record (must enter n then <CR>)')
      disp(' ')
      disp('The following commands do not exit RSX:')
      disp('  c    copy current window to all traces')
      disp('  t    return x and y values of current location, plot o there')
      disp('  p    print current plot in tall format')
      disp('  l    print current plot in landscape format')
      disp('  x    write travel time curve phase name nearest the cursor')
      disp('  t    write travel time at cursor')
      disp('  m    enter move a trace mode:')
      disp('          use left mouse to select a new trace')
      disp('          center or right mouse to move the trace')
      disp('          <CR> to apply all moves to D1')
      disp('          or ''q'' to quit plot 1 and not apply moves to D1')
      disp('  f    enter flip a trace mode:')
      disp('          use any mouse button to choose traces to flip')
      disp('          traces are flipped and redisplayed after <CR>')
      disp('          mouse buttons are ignored if character q is entered')
      disp('  k    enter kill (delete) a trace mode:')
      disp('          use any mouse button to select traces to delete')
      disp('          traces are deleted from Data1 after <CR>')
      disp('          mouse buttons are ignored if character q is entered')
      disp('  d    deglitch trace from current file:')
      disp('          asks for mouse input...if LB used, user must input')
      disp('           number of glitches to remove; if RB used, removes')
      disp('           largest glitch...in either case removes glitch from')
      disp('           trace closest to mouse input')
      disp(' ')
      disp('Summary of mouse commands:')
      disp('  (Note:  LB, CB, RB are left, center and right mouse buttons)')
      disp('    (1) When picking first window;')
      disp('           LB, CB or RB used to pick window limits...')
      disp('    (2) For all successive windows;')
      disp('           LB -> choose left window limit')
      disp('           RB -> choose right window limit')
      disp('           CB -> choose window center')
      disp('    (3) to escape from window selection;')
      disp('           <CR> -> continues analysis')
      disp('             r  -> replot current file')
      disp('             q  -> exit and quit')
      disp(' ')
    else
      disp('command not recognized, try h for list of commands...')
    end;
  end;
end;
