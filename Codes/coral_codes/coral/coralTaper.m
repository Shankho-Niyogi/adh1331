...
% KCC fixed bug cmpstr -> strcmp  6/27/2007
...
if nargin == 1;
  ...
  if strcmp('struct',class(opt));
    indx = find( strcmp('taper',fields(opt)) );
    if length(indx)>0;  
...
taper_fract=min(0.5,taper_fract);  % taper can't be more than 50% off each side

for idata = 1 : ndata;             % loop over seismograms
  temp_data = data(idata).data;    % input seismogram
  if length(temp_data)>1;
    if taper_fract>=0;
      taper = taper_fract;
    else
      duration = data(idata).recSampInt * data(idata).recNumData;   % duration of data (s)
      taper  =  min(0.5, abs(taper_fract)/duration);
    end
    data(idata).data   = taperd(temp_data,taper);
