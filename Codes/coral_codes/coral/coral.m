...
%    Coral version 3.0    10/28/2004 - MATLAB 6.5
...
% JCC2
 ...
 elseif abs(opt(1:4))==abs('corr')
   ...
   elseif n_smat==2,
     ...
     %cmt_file_name=[ CORALHOME '/quakes/harvard.cmt.dek'];
   end
...
if exist('obs1')==1,
  ...
  if coralfun;
    if exist('FF'); FF_tmp=FF; else; FF_tmp=[];  end
    if ~exist('wind_save'); wind_save=[];end
    [out_opts, out_matrix, out_phase, out_station] = ...
    coral_fun_end(data1,header1,obs1,label1,Loc,Record,Station,Delta,FF_tmp,wind_save);
    if any(strcmp(fieldnames(opts),'eval_at_end')); % if this string exists, evaluate it
      eval(opts.eval_at_end);
    end
  end
end
