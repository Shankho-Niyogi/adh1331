...
if length(s_fill)==2;
  ...
  for i=1:m
    ttemp=t_start(i)+[0:n_time_samples(i)-1]*sintr(i); indx=istart(i):iend(i);
    h_plot(i)=plot(ttemp,d(indx,i)*sca/drange(i)+delta(i),'-w','EraseMode','normal'); 
    if i==1, hold on; set(gca,'drawmode','fast');end
  end
end
...
if length(syn)>0,
  ...
  if (length(yval) == 1 | ...     % jpw: This allows plotting of tt curves for only one seismogram.
	...
	ymax0 = max(ytmp(:,2));
	for isyn = 1:n_syn
	  h_tt = plot([syn1(isyn,1) syn1(isyn,1)],[ymin0 ymax0],':w');
	end
  else
	h_tt = plot(syn1',yval,':w');
  end
...
% plot point, return time and amplitude...
    ...
    elseif setstr(button) == 't',
      ...
      plot(xx,delta(traceno),'yo');
       xx + header(1,traceno) - t_offset(traceno) - syn(:,traceno)
